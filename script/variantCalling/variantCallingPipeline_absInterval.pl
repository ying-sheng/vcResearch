...
my $captureKitFile = $configuration->val("Sample","captureKit");

my $intervalFile = $captureKitFile;
my $intervalTarget = $captureKitFile;
my $intervalBait = $captureKitFile;
...
my @bamFiles = `ls /condor_sh/projects/$project/$sampleID/mapping*/results/*.posiSrt.bam`;
...
system "rsync -ruap /condor_sh/projects/$project/$sampleID/*.pdf 000_fastqFilesAndQC";
...
if(($bRealign == 2) and ($bMark == 2) and ($bBqsr == 2)){
  ...
  system "perl $scriptPath/steps/refineAlignment_absInterval.pl $bamFile $refineAlignCode $configFile 1";
